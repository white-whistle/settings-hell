				...
				<Vertical>
						...
						error={
							<Vertical className="items-center gap-4">
								<Typography color="error">
									This section require you agree to its terms of service
								</Typography>
								<Button
									variant="outlined"
									color="error"
					...
					>
						...
						<Typography variant="subtitle1">
							An important ant storage buffer, some settings may use ants from
							this buffer
						</Typography>
						<AntBuffer />
					</G.If>
...
const AntBuffer = () => {
	...
	return (
		<Vertical>
			{state.map((s, i) => {
				const checked = s;
				const enabled = state?.[i - 1] || (!state.every(Boolean) && i === 0);
							...
							<Switch
								...
								onClick={() =>
									updateGameState((draft) => {
										if (i > 0) {
											draft.general.antBuffer[i - 1] = false;
										}
										draft.general.antBuffer[i] = !draft.general.antBuffer[i];
									})
								}
							/>
						}
